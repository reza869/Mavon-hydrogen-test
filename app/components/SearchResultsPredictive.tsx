...
function SearchResultsPredictiveProducts({
  ...
  return (
    <div className="predictive-search-result" key="products">
      <h5 className="text-[1.2rem] font-medium text-foreground-50 uppercase tracking-wide mb-4">Products</h5>
      <ul className="space-y-4">
        {products.map((product) => {
          ...
          return (
            <li key={product.id}>
              <Link to={productUrl} onClick={closeSearch} className="flex items-center gap-4">
                {image && (
                  <Image
                    alt={image.altText ?? ''}
                    src={image.url}
                ...
                <div>
                  <p className="text-[1.4rem] font-medium text-link">{product.title}</p>
                  {price && <p className="text-[1.4rem] text-foreground-50"><Money data={price} /></p>}
                </div>
              </Link>
            </li>
          );
...
function SearchResultsPredictiveEmpty({
  ...
  return (
    <p className="py-6 text-foreground-50 text-[1.4rem]">
